...
void draw() {
  ...
  // change object mass
  float s5 = cp5.getController("object mass").getValue();
  mSpring.b().mass(s5);
  ...
  if (drop) {
    ...
    //draw particle
      if (checkbox2.getArrayValue()[1] == 1) {
        ...
        offscreen.ellipse(mSpring.b().position().x - projection_correction, mSpring.b().position().y - projection_correction, 15, 15);
      }
    //draw the cubes
