...
class TCPServer extends Server {
  ...
  public void moveToTargetCommand(String toioName, int tx, int ty, int tdeg, int targetingMode, int rotMode, int maxSpeed, int timeOutSec, int speedChangeType) {
   
    
    Client client = _map_toioname_client.get(toioName);

    String msg = "moveToTarget,"+toioName+","+tx+","+ty+","+ tdeg+","+targetingMode+","+rotMode+","+ maxSpeed+","+timeOutSec+","+ speedChangeType + "::";
    client.write(msg);
    println("send moveToTarget command: "+ msg);
  }
  ...
  public void setMatConfig(Client c) {
    String msg = "MATCONFIG,"+minCoordX+","+maxCoordX+","+minCoordY+","+maxCoordY +","+ matCol+","+matRow + "::";
    c.write(msg);
  }
