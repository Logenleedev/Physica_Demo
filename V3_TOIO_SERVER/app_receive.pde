  ...
  if (c !=null) {
    ...
    for (int i = 0; i<s.length; i++) {
      ...
      if (m[0].intern() == ("lost").intern() && m.length > 0) { //register.charAt(0) == m[0].charAt(0)){
        ...
        int cubeId = getIndextoToioID(int(m[1]));
        int left = constrain(int(m[2]), -maxMotorSpeed, maxMotorSpeed);
        int right = constrain(int(m[3]), -maxMotorSpeed, maxMotorSpeed);
        int dur = int(m[4]);
      ...
      } else if (m[0].intern() == ("moveToTarget").intern() && m.length > 5) { //register.charAt(0) == m[0].charAt(0)){
        int cubeId = getIndextoToioID(int(m[1])); //msg.get(0).intValue();
        int tx = int(m[2]); 
        int ty = int(m[3]); 
        int tdeg = int (m[4]);
        int targetingMode = int(m[5]);
        int rotMode = int(m[6]);
        int maxSpeed = int (m[7]);
        int timeOutSec = int (m[8]); // 1-255 sec, 0 = 10 sec
        int speedChangeType = int (m[9]);
        //int distanceMap = int(m[4]); 
        //int offsetMove = int(m[5]); 

        //println("[Server received from App] moveto: id " + cubeId + ", tx,ty " + tx+ ", "  + ty+ ", distanceMap, offsetMove " + distanceMap + ", "+ offsetMove);

        serverToio.moveToTargetCommand("toio"+str(cubeId), tx, ty, tdeg, targetingMode, rotMode, maxSpeed, timeOutSec, speedChangeType);
