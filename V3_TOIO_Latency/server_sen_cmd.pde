...
void motorControl(int cubeId, float left, float right, int duration) {
  ...
  cubes[cubeId].currentMotL = left;
  cubes[cubeId].currentMotR = right;
  
  sendCommand += msg;
  
  cubes[cubeId].targetMode = false;
...
void moveTo(int cubeId, int tx, int ty, int dM, int oM) {
  ...
  cubes[cubeId].targetMode = true;

  
  
}

void moveToTarget(int cubeId, int tx, int ty, int tdeg, int targetingMode, int rotMode,  int maxSpeed, int timeOutSec, int speedChangeType) {
  // command to make cube move to target coordinate
  // tx, ty => target coordinate
  // dM (distanceMap) => Ratio of Distance mapped to speed  (around 100 is good)
  // oM (offsetMotion) => offset for target position (around 10 is good)
  String msg = "moveToTarget,"+cubeId+ "," +tx+","+ty+","+ tdeg+","+targetingMode+","+rotMode+","+ maxSpeed+"," + timeOutSec+","+speedChangeType+"\n";
  sendCommand += msg;
}
