...
void setup() {
  ...
  ViscousDrag myDrag = new ViscousDrag(0.3f);
  mPhysics.add(myDrag);
...
void draw() {
  ...
  if (drop) {
    ...
    if (cubes[0].isLost==false && cubes[0].p_isLost == true) {//when toios are placed
    ...
    if (cubes[0].isLost==false) {
      ...
      if ( eventDetection(cubes[0].x, cubes[0].y, cubes[0].prex, cubes[0].prey, cubes[0].speedX, cubes[0].speedY) ) {
        println("cube location: " + cubes[0].x + " " + cubes[0].y);
        println("cube pre location: " + cubes[0].prex + " " + cubes[0].prey);
        println("particle speed: " + mSpring.a().velocity().x + " " + mSpring.a().velocity().y);
        mSpring.a().position().set(cubes[0].x, cubes[0].y);
    ...
    if (cubes[1].isLost==false) {
      ...
      if ( eventDetection(cubes[1].x, cubes[1].y, cubes[1].prex, cubes[1].prey, cubes[1].speedX, cubes[1].speedY) ) {
        mSpring.b().position().set(cubes[1].x, cubes[1].y);
